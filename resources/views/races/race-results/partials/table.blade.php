    ...
    <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
        <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
            @if($tempResults->count() > 0 && auth()->user() && auth()->user()->can('manage-races'))
                @include('races.partials.temp-results', ['submitRoute' => 'race.results.store', 'deleteRoute' => 'race.temp-results.destroy'])
            @elseif($race->results->count() === 0 && auth()->user() && auth()->user()->can('manage-races'))
                @include('races.race-results.partials.upload-form', ['route' => 'race.temp-results.store'])
            @else
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                        ...
                        @foreach($race->results as $result)
                            <x-race-table-row :driver="$result->driver">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div
                                        class="text-sm text-gray-900 dark:text-white">{{ $result->num_pit_stops }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
